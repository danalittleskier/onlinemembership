...
public class AuthorizeNetCreditCardProcessingServiceImpl implements CreditCardProcessingService {
	...
	public void voidTransaction(String transactionId) throws Exception {
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug("Gateway Response: " + line);
	...
	private GatewayResponse sendGatewayRequest(AccountBean accountBean) throws Exception {
		...
		params.append("x_delim_char=").append(DELIMITER).append("&");
		params.append("x_relay_response=FALSE&");
		params.append("x_allow_partial_auth=FALSE&");
		...
		if (log.isDebugEnabled()) {
			...
			// log.debug("GatewayResponse SubCode: " +
			// response.getResponseSubCode());
			// log.debug("GatewayResponse Reason Code: " +
	...
	private static class GatewayResponse {
		...
		private String balanceOnGiftCard;
		...
		GatewayResponse(String responseString) {
			...
			balanceOnGiftCard = splitResponse.get(54);
		...
		String getBalanceOnGiftCard() {
			return balanceOnGiftCard;
		}
		...
		private List<String> split(String pattern, String in) {
			...
			while (true) {
				...
				if (s2 != -1) {
					...
					// the end part of the string (string not pattern
					// terminated)
